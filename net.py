...
class ConvLSTM(chainer.Chain):
    def __init__(self, width, height, in_channels, out_channels, batchSize=1):
        ...
        for nth in range(len(self.in_channels)):
            self.add_link('x_i' + str(nth), L.Convolution2D(self.in_channels[nth], out_channels, 8, stride=4, bias=0.1))
            self.add_link('x_f' + str(nth), L.Convolution2D(self.in_channels[nth], out_channels, 8, stride=4, bias=0.1))
            self.add_link('x_c' + str(nth), L.Convolution2D(self.in_channels[nth], out_channels, 8, stride=4, bias=0.1))
            self.add_link('x_o' + str(nth), L.Convolution2D(self.in_channels[nth], out_channels, 8, stride=4, bias=0.1))
